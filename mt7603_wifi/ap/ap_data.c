	...
	*/
	...
	if (tr_entry->EntryType == ENTRY_CAT_MCAST)
	{
		...
		if (InIgmpGroup)
		{
			...
			{
#ifdef MWDS
				if (VALID_WCID(wcid) && pEntry && pEntry->bEnableMWDS) {
...
INT ap_rx_pkt_allow(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	RXINFO_STRUC *pRxInfo = pRxBlk->pRxInfo;
	HEADER_802_11 *pHeader = pRxBlk->pHeader;
#if defined(WDS_SUPPORT) || defined(CLIENT_WDS) || defined(IDS_SUPPORT) || defined(MWDS) || defined(WH_EVENT_NOTIFIER)
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1))
	{
		...
		if(pEntry && IS_MWDS_OPMODE_AP(pEntry))
        {
			...
			MWDSProxyTabUpdate(pAd, pEntry->func_tb_idx, pEntry->wcid, pHeader->Octet, ARPSenderIP);
        }
        else
            pEntry = NULL;
#endif /* MWDS */
	...
	if (((pFmeCtrl->FrDs == 0) && (pFmeCtrl->ToDs == 1))) {
		if((pFmeCtrl->SubType != SUBTYPE_DATA_NULL) && (pFmeCtrl->SubType != SUBTYPE_QOS_NULL)) {
			if(pEntry && IS_MWDS_OPMODE_AP(pEntry)){
				...
				{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, WARN,("APRxPktAllow: Non MWDS Pkt=> wdev_idx=0x%x, wdev_type=0x%x, func_idx=0x%x \nEth Hdr: Dest[%02x-%02x-%02x-%02x-%02x-%02x] Source[%02x-%02x-%02x-%02x-%02x-%02x] Type[%02x-%02x]\n",
