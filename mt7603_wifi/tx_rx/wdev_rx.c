...
#ifdef MWDS
		&& (pEntry && (!IS_ENTRY_MWDS(pEntry)))	/*add mwds disable code here*/
#endif
		)
		{
...
//+++Add by shiang for work-around, should remove it once we correctly configure the BSSID!
		...
		if (rxd_base->rxd_2.icv_err) {
			...
			/*
			  *if exist mwds-client connection, don't print ICV error BC/MC packets
			  *because MWDS-client don't do group rekey action
			  */
			if (pEntry && pEntry->bEnableMWDS &&
				(pRxBlk->pHeader->FC.FrDs == 1 && pRxBlk->pHeader->FC.ToDs == 0)) {
