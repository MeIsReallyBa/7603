...
INT32 APAssocRespTxSHandler(RTMP_ADAPTER *pAd, CHAR *Data, UINT32 Priv)
{
    ...
    DBGPRINT(RT_DEBUG_OFF,("===[%s]: (RE=%d, LE=%d, ME=%d), wlan_idx = %d\n",__FUNCTION__, 
    ...
    if ((txs_d0->RE == 0) && (txs_d0->LE == 0) && (txs_d0->ME == 0))
    {
        ...
        if(pEntry) 
		{
            ...
            DBGPRINT(RT_DEBUG_OFF,("===[%s]: wlan_idx = %d,AuthAssocNotInProgressFlag = %d pEntry->bssid %02x:%02x:%02x:%02x:%02x:%02x \n",
		          __FUNCTION__, txs_d3->wlan_idx,pEntry->AuthAssocNotInProgressFlag, PRINT_MAC(pEntry->bssid)));
        }
    } else {
        DBGPRINT(RT_DEBUG_OFF,("===[%s] status  error returned \n", __FUNCTION__));
    }
    return 0;
}
