	...
	IN BOOLEAN is_ap)
{
	...
	if (mac_entry) {
		...
		if (is_ap) {
			...
			apcli_entry = &adapter->StaCfg[mac_entry->func_tb_idx];
...
)
{
	...
	if (is_ap) {
		if (IS_MAP_ENABLE(entry->wdev) && (entry->DevPeerRole & BIT(MAP_ROLE_BACKHAUL_STA)))
			return a4_ap_peer_enable(adapter, entry, A4_TYPE_MAP);
	}
#ifdef APCLI_SUPPORT
	else {
		apcli_entry = &adapter->StaCfg[entry->func_tb_idx];
		if (IS_MAP_ENABLE(&apcli_entry->wdev) &&
...
)
{
	if (is_ap)
		...
		return a4_apcli_peer_disable(adapter, &adapter->StaCfg[entry->func_tb_idx], entry, A4_TYPE_MAP);
#else
	return FALSE;
#endif
}
