	...
	IN UCHAR ifIndex)
{
	...
	pMbss = &pAd->ApCfg.MBSSID[ifIndex];
	if (!pMbss->bMWDSAPInit) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: MWDS not init\n", __func__));
		return;
	}
	...
	IN PUCHAR pExcludeMac)
{
	...
	UCHAR Wcid = 0;
	BSS_STRUCT *pMbss = NULL;
	UCHAR Rwcid = 0, wcid_list[MAX_LEN_OF_MAC_TABLE] = {0};
	UCHAR widx = 0, wcnt = 0;

#if ((defined(WH_EZ_SETUP) && defined(EZ_DUAL_BAND_SUPPORT)) || ( defined(MWDS) && defined(IGMP_SNOOP_SUPPORT) && defined(IGMP_TX_QUERY_HOLD) ))
	...
	if (ap_wdev->bSupportMWDS && ap_wdev->IgmpSnoopEnable) {
	...
	if (MWDSGetConnEntryCount(pAd, ifIndex) > 0) {
		...
		{
			if (pConnEntry && pConnEntry->Valid && VALID_WCID(pConnEntry->wcid)) {
				wcid_list[wcnt++] = pConnEntry->wcid;
				if (wcnt == MAX_LEN_OF_MAC_TABLE) {
					DBGPRINT(RT_DEBUG_ERROR, ("%s exceeds %d repeater added! not possible\n",
							__func__, wcnt));
					break;
				}
			}
		...
		for (widx = 0; widx < wcnt; widx++) {
			Rwcid = wcid_list[widx];

			if (bFound && (Wcid == Rwcid))
				continue;

			pEntry = &pAd->MacTab.Content[Rwcid];
			if (!pEntry)
				continue;
			wdev = pEntry->wdev;
			if (!wdev || !wdev->if_dev)
			...
			if ((IS_EZ_SETUP_ENABLED(wdev))
				...
				if (drop_pkt_to_dup_ez_apcli && pEntry->link_duplicate) {
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						("%s %s\n", "MWDSSendClonePacket: Skip sending Group packet to my",
						"CLI as it's other band CLI connected to my other AP"));
					continue;
				} else{
	...
	IN	PSTRING arg)
{
	...
	for (widx = 0; widx < wcnt; widx++) {
		Rwcid = wcid_list[widx];
		count = 0;
		pProxyMac = pAd->MacTab.Content[Rwcid].Addr;
		DBGPRINT(RT_DEBUG_OFF, ("Proxy Mac: %02X:%02X:%02X:%02X:%02X:%02X\n",
			PRINT_MAC(pProxyMac)));
		if (GetRoutingEntryAll(pAd, ifIndex, Rwcid, ROUTING_ENTRY_MWDS,
	...
	PSTRING buffer)
{
	...
	if (RTMPGetKeyParameter("ApCliMWDS", tmpbuf, 256, buffer, TRUE)) {
		...
		for (i = 0, tmpptr = rstrtok(tmpbuf, ";"); tmpptr; tmpptr = rstrtok(NULL, ";"), i++) {
			...
			DBGPRINT(RT_DEBUG_OFF, ("ApCliMWDS=%d\n", Value));
		}
	}
#endif /* APCLI_SUPPORT */
}
