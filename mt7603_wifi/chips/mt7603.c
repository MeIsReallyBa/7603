...
void mt7603_set_ed_cca(RTMP_ADAPTER *pAd, BOOLEAN enable)
{
	...
	else	
	{
		...
		/* When Close EDCCA Function, we close both EDCCA and PLD Function,
		we can not open PLD function only, so, We open EDCCA/PLD but set CR_CCAED_BW20_TH=0x34
		which will never trriger ED Block.
		*/
		macVal = 0xD7e83410;  
		RTMP_IO_WRITE32(pAd, WF_PHY_BASE + 0x0618, macVal);
